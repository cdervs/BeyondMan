...
public class GameStateManager : MonoBehaviour
{
  ...
  public void LoadGameMenu()
  {
    ...
    if (Directory.Exists(Application.persistentDataPath))
        {
            string progressFolder = Application.persistentDataPath;
 
            DirectoryInfo d = new DirectoryInfo(progressFolder);
            foreach (var file in d.GetFiles())
            {
              savedfiles++;
              fileButton = Instantiate(_filesButtonPrefab) as Button;
              fileButton.transform.SetParent(_loadButtonContainer.transform, false);
              fileButtonText = fileButton.GetComponentInChildren<TMP_Text>();
              fileButtonText.text = "File" + savedfiles.ToString();
              Debug.Log(file);
            }
        }
        else
